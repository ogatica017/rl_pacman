...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            new_v = self.values.copy() # create a copy to not overwrite
            for state in states:
                max_val = float('-inf')# possibly self.values[state] instead?
                for action in self.mdp.getPossibleActions(state):
                    curr_val = self.computeQValueFromValues(state, action)
                    if curr_val > max_val:
                        max_val = curr_val
                        new_v[state] = curr_val
            self.values = new_v
    ...
    def computeActionFromValues(self, state):
        ...
        for action in self.mdp.getPossibleActions(state):
            if self.computeQValueFromValues(state, action) > best_q_val:
                best_action = action
                best_q_val = self.computeQValueFromValues(state, action)
        return best_action
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                max_q, diff = self.difference(state)
                pq.push(state, -diff)
        ...
        for i in range(self.iterations):
            if pq.isEmpty():
                break
            s = pq.pop()
            self.values[s] = self.difference(s)[0]

            for p in predecessors[s]:
                max_q, diff = self.difference(p)
                if diff > self.theta:
                    pq.update(p, -diff)
